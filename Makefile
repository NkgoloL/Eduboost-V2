...
test-coverage:
	$(PYTEST) -c pytest-coverage.ini tests/unit -n auto -m "$(PR_TEST_MARKERS)" --cov-fail-under=$(COVERAGE_THRESHOLD) -q
...
diagnostics-dynamic-repository-boundary-repair:
	PYTHONPATH=. $(PYTHON) scripts/patch_diagnostics_dynamic_repository_boundary.py
...
diagnostics-dynamic-repository-boundary-check:
	PYTHONPATH=. $(PYTHON) scripts/check_diagnostics_dynamic_repository_boundary.py
...
backend-implementation-1351-1390R-full-check: diagnostics-dynamic-repository-boundary-repair diagnostics-dynamic-repository-boundary-check diagnostics-dynamic-repository-boundary-test
	$(PYTHON) -m compileall -q app/api_v2_deps app/api_v2_routers app/services scripts tests
	$(PYTHON) -m ruff check app/api_v2_deps/diagnostic_repositories.py app/api_v2_routers/diagnostics.py app/services/diagnostic_domain_service.py scripts/patch_diagnostics_dynamic_repository_boundary.py scripts/check_diagnostics_dynamic_repository_boundary.py tests/unit/test_diagnostics_dynamic_repository_boundary.py --select F821,F401,F811,E402
...
docs-review-dates-check:
	$(PYTHON) scripts/maintenance/check_doc_review_dates.py --root . --canonical-only
...
.PHONY: rr010-beta-outcome-audit rr010-beta-outcome-check
rr010-beta-outcome-audit:
	PYTHONPATH=. python3 scripts/beta_outcomes/audit_rr010_beta_outcome_reporting.py --json
...
.PHONY: prd500-504-popia-live-data-privacy-ops-foundation-check
prd500-504-popia-live-data-privacy-ops-foundation-check:
	PYTHONPATH=. python3 scripts/roadmap_reconciliation/verify_prd500_504_popia_live_data_privacy_ops_foundation.py --authority-only --json
...
.PHONY: prd700-observability-sre-incident-readiness-foundation-check
prd700-observability-sre-incident-readiness-foundation-check:
	PYTHONPATH=. python3 scripts/roadmap_reconciliation/verify_prd700_704_observability_sre_incident_readiness_foundation.py --json

.PHONY: prd700-observability-sre-incident-readiness-foundation-audit
prd700-observability-sre-incident-readiness-foundation-audit:
	PYTHONPATH=. python3 scripts/production_readiness/audit_prd700_704_observability_sre_incident_readiness_foundation.py --json
...
# PRD-10.5-10.9 controlled beta final live-traffic handoff
.PHONY: prd1005-1009-controlled-beta-final-live-traffic-handoff-check
prd1005-1009-controlled-beta-final-live-traffic-handoff-check:
...
.PHONY: advisory-quality-gates prd1100r-runtime-restore-5-verify
advisory-quality-gates:
	PYTHONPATH=. python3 scripts/advisory_suites/verify_advisory_quality_gates.py --json
...
.PHONY: prd1100r-runtime-restore-execution-4-verify prd1100r-runtime-restore-execution-4-run
prd1100r-runtime-restore-execution-4-verify:
	PYTHONPATH=. python3 scripts/advisory_suites/verify_generated_contract_frontend_quality_green_evidence.py --json
	PYTHONPATH=. python3 scripts/roadmap_reconciliation/verify_prd1100r_runtime_restore_execution_4_frontend_quality_defect_repair_generated_contract_green_evidence.py --authority-only --json
