...
export function Stars() {
  const seededValue = (index: number, multiplier: number, offset = 0) => {
  ...
  return <div className="stars-bg">{stars.map((s) => <div key={s.id} className="star" style={{ left: `${s.x}%`, top: `${s.y}%`, width: `${s.size}px`, height: `${s.size}px`, "--d": s.dur, "--o": s.opacity, animationDelay: s.delay } as StarStyle} />)}</div>;
}

export function Landing({ onStart, onParent }: { onStart: () => void; onParent: () => void }) {
...
export function Onboarding({ onComplete }: { onComplete: (payload: { grade: number | null; avatar: number | null; nickname: string; language: string }) => void }) {
  const [step, setStep] = React.useState(0);
  const [grade, setGrade] = React.useState<number | null>(null);
  const [avatar, setAvatar] = React.useState<number | null>(null);
...
export function ParentGateway({ onBack }: { onBack: () => void }) {
