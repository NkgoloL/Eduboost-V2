...
class ConsentService:
    ...
    async def require_active_consent(self, learner_id: UUID, db: AsyncSession) -> ParentalConsent:
        consent = await _consent_repo.get_active(learner_id, db)
        if consent is None:
            raise ConsentRequiredError("Active parental consent is required to access learner data.")
        return consent

    async def get_expiring_consents(self, db: AsyncSession, days: int = 30) -> list[ParentalConsent]:
