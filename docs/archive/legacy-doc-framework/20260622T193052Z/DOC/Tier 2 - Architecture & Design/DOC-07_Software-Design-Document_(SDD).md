...
The backend uses layered modular-monolith design:

```text
FastAPI routers -> service/application layer -> repositories -> SQLAlchemy models -> PostgreSQL
                        |                         |
                        +--> domain contracts      +--> Alembic migrations
Frontend Next.js -> typed API service layer -> `/api/v2` backend
Redis -> cache, token revocation and ARQ jobs
```
...
| Layer | Pattern | Notes |
|---|---|---|
| `app/api_v2.py` | Composition root | Middleware, router registration, lifespan, health and metrics. |
| `app/api_v2_routers/` | Thin HTTP adapter | Validate request, enforce dependencies, call services. |
| `app/services/` | Application workflows | Diagnostic, lesson, content, POPIA, audit, AI and study-plan workflows. |
| `app/repositories/` | Persistence adapters | Database access should be contained here where possible. |
| `app/domain/` | Contracts | Pydantic models and domain schemas. |
| `app/models/` | ORM state | SQLAlchemy tables and relationships. |
| `app/modules/` | Bounded modules | Practice, progress, auth, parent portal, gamification and governance areas. |

## Frontend design elements

| Area | Source | Responsibility |
|---|---|---|
| App routes/components | `app/frontend/src/` | Learner, parent, auth, admin and content workflows. |
| API services | `app/frontend/src/lib/api/` | Centralised API calls and route contract. |
| Offline/PWA | Serwist/Dexie dependencies | Offline sync and local state where implemented. |
| Quality gates | `pnpm run lint`, `type-check`, `test` | ESLint, TypeScript and Vitest. |

## Design constraints

- Use `/api/v2` and `/v2` as supported API prefixes.
- Keep route handlers thin and services testable.
- Maintain object-level authorisation for learner, parent and admin operations.
- Do not introduce a separate policy graph-search subsystem.
- Treat generated curriculum artifacts as governed inputs, not ad hoc runtime data.
