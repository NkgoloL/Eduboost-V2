...
EduBoost V2 is a FastAPI + Next.js modular monolith with PostgreSQL, Redis/ARQ and governed content artifacts. It is designed for controlled beta progression, not unrestricted production claims.
...
[Next.js Frontend :3050]
      |
      v
[FastAPI app.api_v2 :8000]
  |      |        |        |
  |      |        |        +--> Prometheus metrics / health / readiness
  |      |        +----------> Redis cache, token revocation, ARQ jobs
  |      +-------------------> PostgreSQL via SQLAlchemy + Alembic
  +--------------------------> LLM providers through guarded service layer

[Content Factory artifacts] -> review/provenance gates -> approved learner content
```
...
|---|---|
| Backend runtime | `app.api_v2:app` is canonical. Legacy imports are compatibility only. |
| API prefixes | Routers are registered under both `/api/v2` and `/v2`. |
| Background jobs | ARQ over Redis is the active durable queue direction. Celery/RabbitMQ must not be added for new V2 work. |
| Database | PostgreSQL 16 local/prod-compatible path with Alembic migrations. |
| Frontend | Next.js app with pnpm, TypeScript, Vitest and ESLint. |
| Content | Launch slice is file-backed and governed by Content Factory registry/targets. |
| Security | JWT, Redis revocation, security headers, rate limiting, secret validation and object-level auth. |
| Compliance | POPIA consent and data subject rights are first-class workflows. |

## Architecture risks

- Legacy documentation and stale generated docs can contradict the runtime.
- Compatibility auth paths must be normalised to avoid router/service claim-shape drift.
- Release evidence must be regenerated after any OpenAPI, route, POPIA or content-scope change.
