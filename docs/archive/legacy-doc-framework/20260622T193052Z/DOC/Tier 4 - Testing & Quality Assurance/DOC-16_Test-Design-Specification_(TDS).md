---
title: Test Design Specification (TDS)
...
| Theme | Test design |
|---|---|
| Auth/session | Valid, expired, revoked, malformed and role-mismatched tokens. |
| Learner access | Guardian owns learner; unrelated guardian cannot read/write; admin exception paths. |
| Diagnostics | Start session, select item, respond, recover, update mastery and handle invalid session. |
| IRT quality | Calibration run creation, item-quality read, manual override and policy thresholds. |
| Content Factory | Scope registry read, coverage targets, validation, review, staging seed and promotion gates. |
| POPIA | Consent grant/deny/withdraw/renew, export, erasure, cancellation, correction and restriction. |
| Frontend | API services, accessibility contracts, offline sync, dashboard rendering and error states. |
| Operations | Health/readiness/metrics response and private metrics restrictions in production. |

## Test data strategy

- Use deterministic fixtures for guardians, learners, content scopes and diagnostic items.
- Use launch scope `{launch_scope['scope_id']}` for positive coverage tests.
- Use unsupported grade/subject scopes for negative tests.
- Use synthetic data only; never commit real child/guardian personal information.
