---
title: Quality Metrics Report (QMR)
...
# Quality Metrics Report (QMR)

| Field | Value |
|---|---|
| Document ID | EDB-QMR-022 |
...
## Static metrics from supplied archive

| Metric | Value |
|---|---:|
| Source files in archive | 5099 |
| Backend Python files under `app` | 390 |
| Backend test files | 767 |
| Frontend test/spec files | 44 |
| Statically discovered API route handlers | 205 |
| Alembic revision files | 44 |
| GitHub workflow files | 44 |
| Content Factory scopes | 1 |
| Content Factory coverage targets | 3 |
| Stale term hits in original `docs/DOC` markdown | 243 |

## Quality trend fields to populate per release

| Metric | Target | Evidence source |
|---|---|---|
| Backend fast gate pass rate | 100% required tests pass | `make test-fast` output |
| Frontend unit pass rate | 100% required tests pass | Vitest output |
| OpenAPI drift | 0 drift | `generate_openapi.py --check` |
| Route inventory drift | 0 drift | `generate_route_inventory.py --check` |
| Security critical vulnerabilities | 0 unapproved criticals | dependency/security scan |
| POPIA workflow failures | 0 release-blocking failures | privacy test report |
| Launch content coverage | 100% for active launch slice | coverage APIs and registry validation |
