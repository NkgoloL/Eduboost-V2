---
title: Code Review Checklist and Standards (CRS)
...
# Code Review Checklist and Standards (CRS)

| Field | Value |
|---|---|
| Document ID | EDB-CRS-023 |
...
| Area | Check |
|---|---|
| Scope | PR states whether it touches backend, frontend, DB, POPIA, content, docs or release evidence. |
| Architecture | Routers remain thin; services/repositories/domain boundaries respected. |
| Auth | Protected routes use correct auth context and object-level authorisation. |
| POPIA | Consent/data-rights changes include tests and audit implications. |
| Database | Schema changes include Alembic migration and rollback consideration. |
| API | Route changes include OpenAPI/route inventory regeneration and frontend contract update. |
| Frontend | pnpm scripts pass; accessibility and error states considered. |
| Content | Generated content includes provenance, review, validation and coverage evidence. |
| Security | No secrets, unsafe logs or broad CORS/metrics exposure. |
| Tests | Relevant unit/integration/E2E tests updated. |
| Docs | `docs/DOC` and domain docs remain aligned to EduBoost V2. |

## Review blockers

- Stale DBE policy-advisory references in controlled docs.
- New Celery/RabbitMQ dependency for V2 work without explicit architecture decision.
- POPIA-sensitive data stored or logged without legal basis and redaction.
- Production-readiness claims without fresh evidence.
