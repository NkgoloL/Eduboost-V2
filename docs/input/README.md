---
title: "Documentation Input Directory Overview"
status: archived
owner: documentation-governance
reviewers: [release-management]
audience: reviewer
source_of_truth: false
supersedes: []
superseded_by: docs/archive/imported-inputs/20260622T193052Z/input
last_reviewed: 2026-06-22
review_interval_days: 180
evidence_command: make docs-housekeeping-check
code_anchors: [docs/documentation/migration_manifests]
---
# Archived documentation redirect

Moved by EduBoost documentation housekeeping.

- Original path: `docs/input`
- New path: `docs/archive/imported-inputs/20260622T193052Z/input`
- Reason: Imported input documents archived and removed from active source-of-truth surface.

This path is retained as a redirect stub only. It is not a current source-of-truth document.
