...
set +e
python3 scripts/audit_remediation/verify_backend_fast_runtime_dependencies.py --json --output "$RAW_DIR/runtime_dependency_verification.json" > "$RAW_DIR/runtime_dependency_verification.stdout" 2>&1
VERIFY_RC=$?
python3 scripts/audit_remediation/verify_backend_fast_environment.py --json > "$RAW_DIR/backend_fast_environment.json" 2>&1
ENV_RC=$?
...
(
  cd "$RAW_DIR"
  find . -type f -maxdepth 1 -print0 | sort -z | xargs -0 sha256sum > SHA256SUMS.txt
)
