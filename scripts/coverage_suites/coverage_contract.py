...
def evaluate_empirical_coverage(
    ...
    """Empirically inspect coverage.xml generated by pytest/coverage for real coverage metrics."""
    xml_file = coverage_xml_path or (root / "coverage.xml")
    if not xml_file.exists():
        return {
            "valid": False,
            "error": f"Coverage artifact not found at {xml_file}. Run 'make test-coverage' first.",
            "measured_line_coverage": 0.0,
            "measured_branch_coverage": 0.0,
            "required_floor": min_coverage,
        }
    try:
        import xml.etree.ElementTree as ET
        tree = ET.parse(xml_file)
        cov_root = tree.getroot()
        line_rate = float(cov_root.get("line-rate", "0")) * 100.0
        branch_rate = float(cov_root.get("branch-rate", "0")) * 100.0
        lines_valid = int(cov_root.get("lines-valid", "0"))
        lines_covered = int(cov_root.get("lines-covered", "0"))
        valid = line_rate >= min_coverage
        return {
            "valid": valid,
            "measured_line_coverage": round(line_rate, 2),
            "measured_branch_coverage": round(branch_rate, 2),
        ...
        return {
            "valid": False,
            "error": f"Failed to parse coverage XML {xml_file}: {exc}",
            "measured_line_coverage": 0.0,
            "measured_branch_coverage": 0.0,
            "required_floor": min_coverage,
        }
