...
def load_scope(root: Path, scope_path: str) -> dict[str, object]:
    ...
    if not path.exists():
        raise FileNotFoundError(f"Missing Stage 3 strict-scope config: {scope_path}")
...
def expand_paths(root: Path, config: dict[str, object]) -> list[Path]:
    ...
    for item in strict_paths:
        ...
        if path.is_dir():
            ...
            raise FileNotFoundError(f"Stage 3 strict path does not exist or is not markdown: {item}")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Stage 3 strict documentation tranche scope.")
    parser.add_argument("--root", default=".")
    parser.add_argument("--scope", default="docs/documentation/stage_3_strict_scope.json")
