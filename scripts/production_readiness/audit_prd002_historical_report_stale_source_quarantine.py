...
def audit(root: Path | str = Path(".")) -> dict[str, Any]:
    ...
    if register.get("last_recorded_item") not in {"PRD-0.1", "PRD-0.2"}:
        errors.append("production readiness register last_recorded_item must be PRD-0.1 before capture or PRD-0.2 after capture")
    if register.get("next_authorised_item") not in {"PRD-0.2", "PRD-0.3"}:
        errors.append("production readiness register next_authorised_item must be PRD-0.2 before capture or PRD-0.3 after capture")
