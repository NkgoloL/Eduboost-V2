...
def audit(root: Path | str = Path(".")) -> dict[str, Any]:
    ...
    ALLOWED_LAST_RECORDED_ITEMS = {f"PRD-0.{idx}" for idx in range(1, 11)} | {f"PRD-1.{idx}" for idx in range(0, 10)} | {f"PRD-{idx}" for idx in range(1, 12)} | {f"PRD-11.0R.RUNTIME-RESTORE.EXECUTION-{idx}" for idx in range(1, 10)}
    ALLOWED_NEXT_AUTHORISED_ITEMS = {f"PRD-0.{idx}" for idx in range(2, 11)} | {"PRD-1", "PRD-2"} | {f"PRD-1.{idx}" for idx in range(0, 10)} | {f"PRD-{idx}" for idx in range(1, 12)} | {f"PRD-11.0R.RUNTIME-RESTORE.EXECUTION-{idx}" for idx in range(1, 10)}
    ...
    if register.get("next_authorised_item") not in ALLOWED_NEXT_AUTHORISED_ITEMS:
        errors.append("production readiness register next_authorised_item must be a valid PRD-0.x state, or PRD-1/PRD-2 after PRD-0.10 closure")
