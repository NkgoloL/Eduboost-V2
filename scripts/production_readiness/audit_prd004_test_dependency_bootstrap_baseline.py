...
def audit(root: Path | str = Path(".")) -> dict[str, Any]:
    ...
    allowed_last = {f"PRD-0.{idx}" for idx in range(3, 11)} | {f"PRD-1.{idx}" for idx in range(0, 10)}
    allowed_next = {f"PRD-0.{idx}" for idx in range(4, 11)} | {"PRD-1", "PRD-2"} | {f"PRD-1.{idx}" for idx in range(0, 10)}
