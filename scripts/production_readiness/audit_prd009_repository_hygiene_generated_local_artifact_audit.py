...
def audit(root: Path = Path(".")) -> dict[str, Any]:
    ...
    if register.get("last_recorded_item") not in {"PRD-0.8", "PRD-0.9", "PRD-0.10"} | {f"PRD-1.{idx}" for idx in range(0, 10)} | {f"PRD-{idx}" for idx in range(1, 12)} | {f"PRD-11.0R.RUNTIME-RESTORE.EXECUTION-{idx}" for idx in range(1, 10)}:
