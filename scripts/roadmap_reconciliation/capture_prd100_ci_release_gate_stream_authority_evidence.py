...
from scripts.production_readiness.apply_prd100_ci_release_gate_stream_authority import apply as apply_authority
from scripts.production_readiness.audit_prd100_ci_release_gate_stream_authority import (
    FALSE_KEYS,
    NO_CHANGE_KEYS,
    PRD_ID,
    PRD1_REGISTER,
...
EVIDENCE_DIR = Path("docs/release-evidence/production-readiness/prd-100-ci-release-gate-stream-authority")
...
def update_prd1_register(now: str) -> dict:
    ...
    data["next_authorised_item"] = "PRD-1.1"
    data["last_recorded_at"] = now
    data["status"] = "prd1_0_recorded_prd1_1_authorised"
    data["authority_established_at"] = now
    for item in data.get("prd1_sequence", []):
        if item.get("prd_id") == PRD_ID:
            item["authorised"] = True
            item["status"] = "recorded"
        elif item.get("prd_id") == "PRD-1.1":
            item["authorised"] = True
            item["status"] = "authorised"
        elif item.get("prd_id") not in {PRD_ID, "PRD-1.1"}:
...
def update_production_register(now: str, prd1_register: dict) -> dict:
    ...
    register["next_authorised_item"] = "PRD-1.1"
    register["status"] = "prd_1_0_recorded_prd_1_1_authorised"
    register["prd1_stream_started"] = True
    register["prd1_stream_started_at"] = now
    register["prd1_next_authorised_item"] = "PRD-1.1"
    register["prd1_sequence"] = prd1_register.get("prd1_sequence", [])
    register.setdefault("current_truth", {})["prd1_stream_started"] = True
    register["current_truth"]["prd1_required_ci_release_gate_convergence_started"] = True
    register["current_truth"]["prd1_next_authorised_item"] = "PRD-1.1"
    ...
    # Keep existing PRD-0 boundary and add PRD-2 boundary for the PRD-1 handoff path.
...
def main() -> int:
    ...
    parser.add_argument("--claim-prd100-ci-release-gate-stream-authority", action="store_true")
    parser.add_argument("--prd-owner", required=True)
    parser.add_argument("--target-branch", default="master")
    parser.add_argument("--require-valid", action="store_true")
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    if not args.claim_prd100_ci_release_gate_stream_authority:
        raise SystemExit("must pass --claim-prd100-ci-release-gate-stream-authority")
    ...
    snapshot = authority_snapshot(Path("."), captured_at=now)
    ...
    record.update({
        "prd_id": PRD_ID,
        "stream_id": STREAM_ID,
        "status": "prd1_0_recorded_prd1_1_authorised",
        "prd010_prd0_closure_evidence_handoff_valid": before.get("prd010_prd0_closure_evidence_handoff_valid") is True,
        "prd1_register_created": True,
        "prd1_sequence_registered": True,
        "prd1_stream_authority_recorded": True,
        "prd1_authority_evidence_recorded": True,
        "next_authorised_item": "PRD-1.1",
        "prd1_1_authorised": True,
    ...
    write_json(EVIDENCE_DIR / "production_readiness_register_prd1_0_snapshot.json", production_register)
    write_json(EVIDENCE_DIR / "prd1_ci_release_gate_register_snapshot.json", prd1_register)
    write_json(EVIDENCE_DIR / "prd1_ci_release_gate_stream_authority_snapshot.json", snapshot)
    (EVIDENCE_DIR / "evidence_index.md").write_text(
        "# PRD-1.0 CI/Release-Gate Stream Authority Evidence\n\n"
        f"- Captured at: {now}\n"
        ...
        f"- PRD-0.10 valid: {after.get('prd010_prd0_closure_evidence_handoff_valid')}\n"
        f"- PRD-1 sequence registered: {after.get('prd1_sequence_registered')}\n"
        f"- Next authorised item: {after.get('register_next_authorised_item')}\n"
        "- CI workflow changes performed by PRD-1.0: false\n"
        "- Required-check enforcement performed by PRD-1.0: false\n"
        "- Release gate enforcement performed by PRD-1.0: false\n"
        "- PRD-2 implementation performed by PRD-1.0: false\n"
    ...
    if args.json:
        ...
        print(f"PRD-1.0 evidence valid: {after.get('valid')}")
