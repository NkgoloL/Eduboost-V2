"""Capture PRD-11.0R.RUNTIME-RESTORE-4 product gate execution evidence."""
...
from scripts.production_readiness.audit_prd1100r_runtime_restore_4_product_gate_execution_critical_flow_repair import ROOT, audit
from scripts.production_readiness.collect_prd1100r_true_state_runtime_baseline import collect_baseline
from scripts.test_suites.product_gate_execution import evaluate_product_gate_execution_contract, flow_command_plan

PRD_ID = "PRD-11.0R.RUNTIME-RESTORE-4"
NEXT_ITEM = "PRD-11.0R.RUNTIME-RESTORE-5"
...
def main() -> int:
    ...
    parser.add_argument("--claim-prd1100r-runtime-restore-4-product-gate-execution-critical-flow-repair", action="store_true")
    ...
    if not args.claim_prd1100r_runtime_restore_4_product_gate_execution_critical_flow_repair:
        raise SystemExit("missing --claim-prd1100r-runtime-restore-4-product-gate-execution-critical-flow-repair")
    before = audit(ROOT)
    ...
    command_plan = flow_command_plan()
    blockers = [
        "Product critical-flow commands have not all been executed to green state in this slice.",
        "Runtime baseline remains red until stack/schema/readiness blockers are cleared.",
        "Operational hold remains active until product_gate_green and runtime_baseline_green are true.",
    ]

    record = _load(RECORD)
    record.update({
        "status": "product_gate_execution_evidence_recorded",
        "product_gate_execution_evidence_recorded": True,
        "product_gate_execution_contract_snapshot": contract,
        "product_gate_command_plan": command_plan,
        "runtime_baseline_snapshot": baseline,
        "known_product_gate_blockers": blockers,
        "captured_at": now,
        ...
        "product_gate_green_reason": "RESTORE-4 records critical-flow execution and blocker evidence; green state requires all independent positive/negative flow command outputs to pass.",
    ...
    for path in (REGISTER, PROD_REGISTER):
        payload = _load(path)
        payload.update({
            "status": "prd11_runtime_restore_4_evidence_recorded",
            "last_recorded_item": PRD_ID,
            "last_recorded_at": now,
            "next_authorised_item": NEXT_ITEM,
            "runtime_restore_4_product_gate_execution_authority_recorded": True,
            "runtime_restore_4_product_gate_execution_evidence_recorded": True,
            "runtime_baseline_green": bool(baseline.get("runtime_baseline_green")),
            "runtime_baseline_status": baseline.get("overall_status"),
            "product_gate_green": False,
            "controlled_beta_activation_operational_hold": True,
        ...
        payload.setdefault("current_truth", {}).update({
            ...
            "prd11_runtime_restore_4_authority_recorded": True,
        ...
        if path == REGISTER:
            ...
            if not any(item.get("prd_id") == PRD_ID for item in sequence):
                sequence.append({"prd_id": PRD_ID, "title": "Product gate execution and critical-flow repair", "status": "evidence_recorded", "authorised": True})
    ...
    provisional_summary = {
        ...
        "product_gate_execution_contract_valid": contract.get("valid"),
        "runtime_baseline_green": bool(baseline.get("runtime_baseline_green")),
        "runtime_baseline_status": baseline.get("overall_status"),
    ...
    _write(EVIDENCE_DIR / "product_gate_execution_contract.json", contract)
    _write(EVIDENCE_DIR / "product_gate_command_plan.json", {"commands": command_plan})
