"""Capture PRD-11.2R script taxonomy evidence."""
...
from scripts.production_readiness.audit_prd1102r_script_taxonomy_functional_overhaul import ROOT, audit
from scripts.script_suites.script_taxonomy import evaluate_taxonomy, script_commands
...
EVIDENCE_DIR = ROOT / "docs/release-evidence/production-readiness/prd-1102r-script-taxonomy-functional-overhaul"
TAXONOMY = ROOT / "docs/roadmap/production_readiness/script_taxonomy.json"
...
def main() -> int:
    ...
    parser.add_argument("--claim-prd1102r-script-taxonomy-functional-overhaul", action="store_true")
    parser.add_argument("--prd-owner", default="Nkgolo Lebelo")
    parser.add_argument("--target-branch", default="master")
    parser.add_argument("--require-valid", action="store_true")
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    if not args.claim_prd1102r_script_taxonomy_functional_overhaul:
        raise SystemExit("missing --claim-prd1102r-script-taxonomy-functional-overhaul")
    before = audit(ROOT)
    ...
    record = _load(RECORD)
    record.update({
        "status": "script_taxonomy_evidence_recorded",
        "script_taxonomy_evidence_recorded": True,
        "taxonomy_snapshot": evaluate_taxonomy(ROOT),
        "script_commands": script_commands(),
        "captured_at": now,
        "prd_owner": args.prd_owner,
        "target_branch": args.target_branch,
        "prd113r_handoff_authorised": True,
        "next_authorised_item": "PRD-11.3R",
    ...
    for path in (REGISTER, PROD_REGISTER):
        payload = _load(path)
        payload.update({
            "status": "prd11_script_taxonomy_evidence_recorded",
            "last_recorded_item": "PRD-11.2R",
            "last_recorded_at": now,
            "next_authorised_item": "PRD-11.3R",
            "prd11_script_taxonomy_authority_recorded": True,
            "prd11_script_taxonomy_evidence_recorded": True,
            "prd113r_handoff_authorised": True,
        })
    ...
    _write(EVIDENCE_DIR / "script_taxonomy.json", after["taxonomy"])
    _write(EVIDENCE_DIR / "script_commands.json", {"commands": script_commands()})
