"""Capture PRD-2.4-2.6 runtime KG route projection behaviour evidence."""
...
EVIDENCE_JSON = ROOT / "docs/roadmap/production_readiness/prd2_runtime_kg_route_projection_behaviour.json"
EVIDENCE_DIR = ROOT / "docs/release-evidence/production-readiness/prd-204-206-runtime-kg-route-projection-behaviour"
...
def capture(owner: str, target_branch: str, require_valid: bool) -> dict[str, Any]:
    ...
    if require_valid and not before.get("authority_valid"):
        raise SystemExit("PRD-2.4-2.6 authority is not valid; refusing evidence capture")
    ...
    record.update({
        "status": "recorded",
        "runtime_kg_route_projection_behaviour_recorded": True,
        "recorded_at": now,
        "recorded_by": owner,
        "target_branch": target_branch,
        "next_authorised_item": "PRD-2.7-2.9",
        "prd2_7_2_9_authorised": True,
    ...
    register.update({
        "status": "runtime_kg_route_projection_behaviour_recorded",
    ...
    prod_register.update({
        "status": "prd2_runtime_kg_route_projection_behaviour_recorded",
        "last_recorded_item": PRD_ID,
        "last_recorded_at": now,
        "next_authorised_item": "PRD-2.7-2.9",
        "prd2_runtime_kg_route_projection_behaviour_recorded": True,
        "prd2_next_authorised_item": "PRD-2.7-2.9",
        "prd3_implementation_authorised": False,
    })
    prod_register.setdefault("current_truth", {})["prd2_runtime_kg_route_projection_behaviour_recorded"] = True
    prod_register["current_truth"]["prd2_next_authorised_item"] = "PRD-2.7-2.9"
    ...
    if require_valid and not after.get("valid"):
        raise SystemExit("PRD-2.4-2.6 final verifier is not valid after capture")
    return after


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--claim-prd204-206-runtime-kg-route-projection-behaviour", action="store_true")
    parser.add_argument("--prd-owner", required=True)
    parser.add_argument("--target-branch", default="master")
    parser.add_argument("--require-valid", action="store_true")
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    if not args.claim_prd204_206_runtime_kg_route_projection_behaviour:
        raise SystemExit("explicit PRD-2.4-2.6 claim flag is required")
