#!/usr/bin/env python3
"""Verify Phase 19 controlled beta activation-preflight authority evidence.

This verifier intentionally does not authorise controlled beta launch, live learner
traffic, deployment, production release, or learner data migration. It verifies
only that the activation preflight package and evidence have been captured after
Phase 18 launch-governance readiness.
"""
...
REQUIRED_TRUE_FIELDS: tuple[str, ...] = (
    "controlled_beta_activation_preflight_claimed",
    "controlled_beta_activation_preflight_recorded",
    "phase_18_controlled_beta_launch_governance_valid",
    "activation_preflight_documents_valid",
)
...
def verify_record(record_path: pathlib.Path = RECORD_PATH) -> dict[str, Any]:
    ...
    if not record_path.exists():
        return {
            ...
            "controlled_beta_activation_preflight_recorded": False,
    ...
    if record.get("slice") != "PHASE-19-CONTROLLED-BETA-ACTIVATION-PREFLIGHT-AUTHORITY":
        errors.append("slice must be PHASE-19-CONTROLLED-BETA-ACTIVATION-PREFLIGHT-AUTHORITY")
    if record.get("status") != "controlled_beta_activation_preflight_recorded":
        errors.append("status must be controlled_beta_activation_preflight_recorded")
    if record.get("beta_scope") != "controlled_beta_activation_preflight_gate":
        errors.append("beta_scope must be controlled_beta_activation_preflight_gate")
    if record.get("scope") != "controlled_beta_activation_preflight_only":
        errors.append("scope must be controlled_beta_activation_preflight_only")
    ...
    if not isinstance(index_value, str) or not pathlib.Path(index_value).exists():
        ...
        for marker in (
            "Phase 19 Controlled Beta Activation Preflight Evidence",
    ...
    result = raw_payloads.get("raw/controlled_beta_activation_preflight_result.json") or {}
    if result.get("valid") is not True:
        errors.append("controlled beta activation-preflight raw result must be valid")
    if result.get("controlled_beta_activation_preflight_recorded") is not True:
        errors.append("controlled beta activation-preflight raw result must record true")
    for field in FALSE_BOUNDARY_FIELDS:
        if result.get(field) is not False:
            errors.append(f"raw result {field} must remain false")

    phase18 = raw_payloads.get("raw/phase18_launch_governance_verification.json") or {}
    if phase18.get("valid") is not True or phase18.get("returncode") != 0:
        errors.append("Phase 18 launch-governance verification must be valid with returncode 0")

    docs = raw_payloads.get("raw/activation_preflight_documents.json") or {}
    if docs.get("valid") is not True:
        errors.append("activation preflight documents raw evidence must be valid")

    git_state = raw_payloads.get("raw/git_state.json") or {}
    if git_state.get("tracked_worktree_clean_before_capture") is not True:
        errors.append("tracked worktree must have been clean before activation-preflight capture")

    if record.get("source_commit") != record.get("remote_target_sha"):
        errors.append("source_commit must match remote_target_sha for final activation-preflight evidence")
    ...
    snapshot = raw_payloads.get("raw/controlled_beta_activation_preflight_record_snapshot.json") or {}
    ...
    return {
        "valid": len(errors) == 0,
        "record": record_path.as_posix(),
        "controlled_beta_activation_preflight_recorded": record.get("controlled_beta_activation_preflight_recorded") is True,
        "phase_18_controlled_beta_launch_governance_valid": record.get("phase_18_controlled_beta_launch_governance_valid") is True,
        "activation_preflight_documents_valid": record.get("activation_preflight_documents_valid") is True,
