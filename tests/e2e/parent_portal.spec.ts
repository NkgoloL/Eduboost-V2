...
test.describe("Parent Portal — Progress Reports", () => {
  ...
  test.beforeAll(() => { const fx = JSON.parse(fs.readFileSync(FIXTURE_FILE, "utf-8")); learnerId = fx.learnerId; learnerName = fx.learnerName ?? fx.learner?.display_name ?? "DevLearner"; });
  test("parent portal dashboard loads and shows learner card", async ({ page }) => { await page.goto("/parent"); await expect(page.getByRole("heading", { name: /parent portal/i })).toBeVisible({ timeout: 10000 }); await expect(page.getByText(learnerName)).toBeVisible({ timeout: 10000 }); });
