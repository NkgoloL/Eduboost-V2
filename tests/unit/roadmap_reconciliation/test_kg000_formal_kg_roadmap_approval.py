...
def copy_payload(tmp_path: Path) -> Path:
    root = tmp_path / "repo"; root.mkdir(); payload = Path(__file__).resolve().parents[3]
    for rel in ["README.md","docs/README.md","docs/architecture/README.md","docs/adr/README.md","docs/roadmap/README.md","docs/roadmap/reconciliation/final_roadmap_reconciliation_closure_record.json"]:
        src = payload / rel
        if src.exists(): (root / rel).parent.mkdir(parents=True, exist_ok=True); shutil.copy2(src, root / rel)
...
def test_authority_audit_passes(tmp_path: Path):
    root = copy_payload(tmp_path); result = audit(root); assert result["authority_valid"], result; assert result["kg_ids"] == [f"KG-{i}" for i in range(9)]

def test_verifier_authority_only_before_capture(tmp_path: Path):
    root = copy_payload(tmp_path); result = evaluate(root); assert result["authority_valid"], result; assert not result["valid"]; assert not result["formal_kg_roadmap_approval_recorded"]

def test_capture_makes_record_valid(tmp_path: Path):
    root = copy_payload(tmp_path); subprocess.run(["git", "init"], cwd=root, check=True, stdout=subprocess.DEVNULL); subprocess.run(["git", "config", "user.email", "test@example.com"], cwd=root, check=True); subprocess.run(["git", "config", "user.name", "Test User"], cwd=root, check=True); subprocess.run(["git", "add", "."], cwd=root, check=True); subprocess.run(["git", "commit", "-m", "init"], cwd=root, check=True, stdout=subprocess.DEVNULL)
    ...
    env = os.environ.copy(); env["PYTHONPATH"] = str(root) + os.pathsep + str(payload)
    ...
    result = evaluate(root); assert result["valid"], result; assert result["runtime_kg_implementation_claimed"] is False

def test_boundary_flags_are_false_in_register(tmp_path: Path):
    root = copy_payload(tmp_path); register = json.loads((root / "docs/roadmap/knowledge_graph/kg_roadmap_register.json").read_text()); assert all(value is False for value in register["boundary"].values())

def test_kg0_does_not_authorise_runtime_kg(tmp_path: Path):
    root = copy_payload(tmp_path); record = json.loads((root / "docs/roadmap/knowledge_graph/kg_000_formal_kg_roadmap_approval_record.json").read_text()); assert record["runtime_kg_implementation_claimed"] is False; assert record["runtime_kg_authority_switch_authorised"] is False; assert record["database_schema_migration_authorised"] is False
