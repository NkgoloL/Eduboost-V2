...
from app.api_v2_routers.privacy_operations import router


@pytest.mark.asyncio
async def test_privacy_operations_routes():
    app = FastAPI()
    app.include_router(router)

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp_readiness = await client.get("/privacy-operations/live-data/readiness")
        ...
        resp_final = await client.get("/privacy-operations/live-data/final-assurance")
